#!/bin/bash
# Test 5: Sustained load - 500 RPM for 2 minutes

set -e
cd "$(dirname "$0")/.."

RESULTS_FILE="tests/test5-results.txt"
...
{
  echo "=========================================="
  echo "TEST 5: SUSTAINED LOAD (500 RPM, 2 minutes)"
  echo "=========================================="
  echo "Timestamp: $(date)"
  echo ""
} | tee "$RESULTS_FILE"
...
# Run load test
API_ENDPOINT="$API_ENDPOINT" RPM=500 DURATION=2 node tests/load-test.js | tee -a "$RESULTS_FILE"
